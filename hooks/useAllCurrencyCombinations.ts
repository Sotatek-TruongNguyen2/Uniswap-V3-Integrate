...
import { ADDITIONAL_BASES, BASES_TO_CHECK_TRADES_AGAINST, CUSTOM_BASES } from '../routing'
...
export function useAllCurrencyCombinations(currencyA?: Currency, currencyB?: Currency): [Token, Token][] {
    ...
    const findBasePairs = (bases: Token[]): [Token, Token][] => {
      return bases
        .flatMap((base: Token): [Token, Token][] => bases.map((otherBase) => [base, otherBase]))
        // though redundant with the first filter below, that expression runs more often, so this is probably worthwhile
        .filter(([t0, t1]: [Token, Token]) => !t0.equals(t1));
    };
  
    const basePairs = findBasePairs(bases);
  
    const deriveAllCurrencies = (tokenA: Token | undefined, tokenB: Token | undefined, bases: Token[], basePairs: [Token, Token][], chainId: number | undefined) => {
