...
const web3Provider = new ethers.providers.JsonRpcProvider(process.env.RPC_URL);
...
export async function useClientSideV3Trade<TTradeType extends TradeType>(
  ...
  const { routes, loading: routesLoading } = await useAllV3Routes(
    currencyIn,
    currencyOut
  );
  ...
  quotesResults = quotesResults.map((quotesResult: any) =>
    toCallState(
      quotesResult,
      new ethers.utils.Interface(QuoterV2ABI),
      quoter?.interface?.getFunction(callDatas[0].callData.substring(0, 10)),
      undefined
    )
  );
