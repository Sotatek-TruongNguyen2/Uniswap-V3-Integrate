...
export async function usePools(
    ...
    const getPoolTokens = (chainId: number, poolKeys: [Currency | undefined, Currency | undefined, FeeAmount | undefined][]): ([Token, Token, FeeAmount] | undefined)[] => {
    ...
    const getPoolAddresses = (chainId: number, poolTokens: ([Token, Token, FeeAmount] | undefined)[]): (string | undefined)[] => {
    ...
    const liquidityDecodedResults = liquidityMultiResults.returnData.map((result: any) => { return { 
        result: result == "0x" ? undefined : POOL_STATE_INTERFACE.decodeFunctionResult("liquidity", result),
        valid: result != "0x"
    }});
