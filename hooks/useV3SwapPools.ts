...
export async function useV3SwapPools(
  ...
  const getAllCurrencyCombinationsWithAllFees = (
    allCurrencyCombinations: [Token, Token][],
    chainId: number | undefined
  ): [Token, Token, FeeAmount][] => {
    return allCurrencyCombinations.reduce<[Token, Token, FeeAmount][]>(
      (list, [tokenA, tokenB]) => {
  ...
  const allCurrencyCombinationsWithAllFees =
    getAllCurrencyCombinationsWithAllFees(
